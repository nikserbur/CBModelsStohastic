...
**Drift функция** представляет собой расширенное правило Тейлора: `μ(r,t) = κ(θ(t) - r(t))`, где `θ(t) = r* + α(π(t) - π*) + β(y(t) - y*) + γΔCA(t) + δS(t)` включает отклонения инфляции от цели, разрыв выпуска, динамику цифровых активов и санкционные индикаторы.

**Диффузионная компонента** использует HAR волатильность: `σ(r,t) = σ₀√HAR(t) × √(1 + λ|r(t) - r*|)`, где `HAR(t) = β₁RV(t-1) + β₂RV(t-1,t-5) + β₃RV(t-1,t-22)` учитывает гетерогенность временных горизонтов.

**α-stable скачки** моделируют экстремальные события: `dJ(t) ~ S(α, β, γ, δ)` с параметрами α = 1.8 (хвосты), β = 0 (симметрия), γ = масштаб, δ = 0 (сдвиг).

**Пуассоновские шоки** представляют редкие структурные разрывы: `dN(t) ~ Compound Poisson(λ, F)` с интенсивностью λ = 0.05 и распределением размера скачка F.
...
Математически система описывается матрицей переходов `P = [0.85 0.10 0.05; 0.20 0.70 0.10; 0.15 0.15 0.70]` и вероятностями эмиссии `p(r(t)|S(t)=k) = N(r(t); μₖ, σₖ²)`. Forward-Backward алгоритм: `α(t,k) = p(r(t)|S(t)=k) × Σⱼ α(t-1,j) × P(j,k)` и `β(t,k) = Σⱼ β(t+1,j) × P(k,j) × p(r(t+1)|S(t+1)=j)` позволяют вычислить вероятности состояний `P(S(t)=k|r₁:T) = α(t,k) × β(t,k) / Σⱼ α(t,j) × β(t,j)`.
...
Основная модель `RV(t) = β₀ + β₁RV^(d)(t-1) + β₂RV^(w)(t-1) + β₃RV^(m)(t-1) + ε(t)` включает временные компоненты: дневную `RV^(d)(t) = RV(t)`, недельную `RV^(w)(t) = 1/5 × Σᵢ₌₁⁵ RV(t-i+1)` и месячную `RV^(m)(t) = 1/22 × Σᵢ₌₁²² RV(t-i+1)`. Прогнозирование осуществляется через `RV(t+h) = β₀ + β₁RV^(d)(t+h-1) + β₂RV^(w)(t+h-1) + β₃RV^(m)(t+h-1)`.
...
**Стандартный R²:** `R²ₛₜd = 1 - SS_res/SS_tot = 1 - Σε²ᵢ/Σ(yᵢ-ȳ)²`

**Taleb точная формула** для t-распределения с df=α: `R²ₜₐₗₑᵦ = a²(α-2)/(a²(α-2) + α)`, где `a² = Var(ŷ)/Var(ε)` и `E[ε²] = α/(α-2)`

**MLE коррекция:** `R²ₘₗₑ = 1 - SS_res × (α/(α-2))/SS_tot`

**Shapley декомпозиция** для важности переменных: `R²ₛₕₐₚₗₑy = Σᵢ φᵢ(R²)`, где `φᵢ(R²) = 1/p! × Σ_S⊆N\{i} [R²(S∪{i}) - R²(S)]`

## Gradient Boosting для выбора распределений

Адаптивный выбор оптимального распределения происходит через градиентный бустинг с функцией потерь `L(θ) = -Σᵢ₌₁ⁿ log p(yᵢ|xᵢ, θ)`, обновлением весов `w^(t+1) = w^(t) × exp(-η × ∇L(θ^(t)))` и финальной моделью `p(y|x) = Σₖ₌₁ᴷ wₖ × pₖ(y|x, θₖ)`, где K = {Normal, t-dist, α-stable, GED}.

## Метрики качества

**Value at Risk (VaR)** на уровне α: `VaRₐ = inf{x : P(L ≤ x) ≥ α}`

**Conditional VaR (Expected Shortfall):** `CVaRₐ = E[L | L ≤ VaRₐ]`

**Continuous Ranked Probability Score (CRPS)** для непрерывных распределений: `CRPS(F,y) = ∫_{-∞}^{∞} (F(x) - 𝟙{x ≥ y})² dx`, для выборочных: `CRPS = 1/n × Σᵢ₌₁ⁿ |xᵢ - y| - 1/(2n²) × Σᵢ₌₁ⁿ Σⱼ₌₁ⁿ |xᵢ - xⱼ|`

**Probability Integral Transform (PIT):** `PIT = F(y|x)`, где F - прогнозное распределение, y - реализация. Идеальная калибровка: PIT ~ U(0,1).
